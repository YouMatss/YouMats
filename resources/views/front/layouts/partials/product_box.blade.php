...
<div class="product-item__outer h-100">
    ...
    <div class="font-size-14">
        @if(is_company())
            <span class="available-product">{{__('product.in_stock')}}</span>
        @else
            @if($product->stock && $product->stock >= $product->min_quantity)
                <span class="available-product">{{__('product.in_stock')}}</span>
            @else
                <span class="OutOfStock-product">{{__('product.out_of_stock')}}</span>
            @endif
        @endif
    </div>
    @endif
    ...
    <div class="product-item__inner">
        ...
        <div class="product-item__body">
            ...
            <div class="mb-2 px-2">
                ...
                <a href="{{ route('vendor.show', [$product->vendor->slug]) }}" class="badge vendor-badge text-white">{{\Str::limit($product->vendor->name, 20)}}</a>
                @endif
...
--}}
            ...
            @if(!$product->category->hide_delivery_status OR ($product->type == 'product' && (!is_company()) && $product->price))
