    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
                ...
                <form method="get" action="">
                    <select name="city_name" class="form-control">
                        ...
                        @foreach(\App\Models\City::all() as $city_loop)
                            <option value="{{$city_loop->id}}" @if($city_loop->name == $city_location) selected @endif>{{$city_loop->name}}</option>
                        @endforeach
                    </select>
                </form>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" style="border-radius: 25px;margin-top: 0">{{__('general.choose')}}</button>
            </div>
        </div>
    </div>
</div>
